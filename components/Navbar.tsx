...
const Navbar: React.FC<NavbarProps> = ({ onNavClick, cartCount, onOpenCart, user, onLoginClick, onLogout, onAdminClick }) => {
    ...
    <>
      ...
      >
        <div className="max-w-[1800px] mx-auto px-8 flex items-center justify-between">
          ...
          <div className={`hidden md:flex items-center gap-12 text-sm font-medium tracking-widest uppercase transition-colors duration-500 ${textColorClass}`}>
            <a href="#products" onClick={(e) => handleLinkClick(e, 'products')} className="hover:opacity-60 transition-opacity">쇼핑</a>
            ...
            <button onClick={onAdminClick} className="hover:opacity-60 transition-opacity uppercase">관리자</button>
          </div>
          ...
          <div className={`flex items-center gap-6 z-50 relative transition-colors duration-500 ${textColorClass}`}>
            ...
            {user ? (
              <div className="flex items-center gap-4">
                <span className="text-sm border-r border-current pr-4 hidden sm:block">{user.username}님</span>
                <button
                  onClick={onLogout}
      ...
      <div className={`fixed inset-0 bg-[#F5F2EB] z-40 flex flex-col justify-center items-center transition-all duration-500 ease-in-out ${mobileMenuOpen ? 'opacity-100 translate-y-0 pointer-events-auto' : 'opacity-0 -translate-y-10 pointer-events-none'
        ...
        <div className="flex flex-col items-center space-y-8 text-xl font-serif font-medium text-[#2C2A26]">
          ...
          <a href="#about" onClick={(e) => handleLinkClick(e, 'about')} className="hover:opacity-60 transition-opacity">브랜드</a>
          <a href="#journal" onClick={(e) => handleLinkClick(e, 'journal')} className="hover:opacity-60 transition-opacity">저널</a>
          <button onClick={onAdminClick} className="hover:opacity-60 transition-opacity text-base uppercase tracking-widest font-sans">관리자</button>
          <button
            onClick={handleCartClick}
          ...
          {user ? (
            <button onClick={onLogout} className="text-base uppercase tracking-widest font-sans">로그아웃</button>
          ) : (
            <button onClick={onLoginClick} className="text-base uppercase tracking-widest font-sans">로그인</button>
          )}
        </div>
